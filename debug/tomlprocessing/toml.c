          ...
          char *path)
{
	...
	char LINE[200], config[100] = "", value[100] = "", key[100] = "";
	char keys[3][100], values[3][100];
	...
	/* TOML file found */
        ...
        while (get_line2 (parameters_file, LINE)) {
            ...
            if (LINE[0] == '#' || LINE[0] == ' ');
                ...
                if (config_bar[1] != 0) {
                    ...
                    extract2 (LINE, config_bar[0] + 1, config_bar[1], config);
                ...
                if (equal[0] != 0) {
                    ...
                    if (chave[0] != 0 && chave[1] != 0) {
                        ...
                        extract2 (LINE, chave[0] + 1, equal[1], keys[0]);

                        /* Extract first value inside values[0] */
                        extract2 (LINE, equal[1] + 1, virg[0], values[0]);

                        /* Extract second key inside keys[1] */
                        extract2 (LINE, virg[0] + 1, equal[2], keys[1]);

                        /* Extract second value inside values[1] */
                        extract2 (LINE, equal[2] + 1, virg[1], values[1]);

                        /* Extract third key inside keys[2] */
                        extract2 (LINE, virg[1] + 1, equal[3], keys[2]);

                        /* Extract third value inside values[2] */
                        extract2 (LINE, equal[3] + 1, chave[1], values[2]);
                        ...
                        /* Extract key */
                        extract2 (LINE, 0, equal[0], key);

                        /* Extract value */
                        if (end < max) end = max;
                        extract2 (LINE, equal[0] + 1, end, value);
                        // TODO: Fix toml parser to handle higher size of LINE
                        printf("%d: %d, %d, %s = %s\n", max, equal[0], end, key, value);
